...
case "${AGENT_E2E_TEST_BOOT_MODE}" in
  "ISO" )
    oc adm node-image create --dir $tmpaddnodedir --registry-config "${PULL_SECRET_FILE}" --loglevel=2

    for (( n=0; n<${NUM_EXTRA_WORKERS}; n++ ))
    do
        sudo virt-xml "${CLUSTER_NAME}_extraworker_${n}" --add-device --disk "$tmpaddnodedir/node.x86_64.iso,device=cdrom,target.dev=sdc"
